...
<table>
  ...
  <tr>
    ...
    <td>
      <ul style="margin: 0; padding-left: 10px;">
        <li>협업 플랫폼 개발 : 협업 시 때로는 비격식적인 의견교류의 필요성이 있다고 생각합니다. 기존 플랫폼들보다 자유로운 형태의 플랫폼을 개발하였습니다.</li>
        <li>Django와 Paas 클라우드를 이용하여 적은 시간 및 개발 비용으로 개발을 완료하였습니다.</li>
      </ul>
    </td>
  ...
  <tr style="font-weight:bold">
    ...
    <td>
      <ul style="margin: 0; padding-left: 10px;">
        ...
        <li>기획부터 개발 과정까지 주도적으로 참여할 수 있어서 회사 생활에서 가장 의미있는 프로젝트입니다.</li>
        <li>문제점 : 새로운 버전의 프로그램을 키오스크에 배포할 시 직접 출장을 가야 하는 경우가 잦았습니다.</li>
        <li>해결 : 키오스크 프로그램 및 사내 프로세스에 특화된 원격 저장소를 구축하고 프로그램의 배포를 자동화해주는 프로그램을 개발하였습니다.</li>
        <li>참여 : 서버, 관리자 페이지, 키오스크 측 응용 프로그램 개발 및 연동 작업에 모두 참여하였습니다.</li>
        <li>기능 : 자체 저장소 구조를 가진 서버를 통해 키오스크에 배포 자동화 및 프로그램의 형상관리, 키오스크 CPU의 제어 및 컨텐츠 시작 및 종료 스케줄링</li>
        <li>연결 유지 : 모듈 간 연결을 유지할 수 있도록 하고, 연결이 끊어지더라도 입력한 명령을 나중에 수행하도록 처리하였습니다.</li>
        <li>결함 내성에 중점을 두고 개발 : 실패 시 재시도, 모듈 간 연결 유지, 배포 오류 시 이전 버전으로 롤백 등에 포커스를 맞춰서 개발하였습니다.</li>
  ...
  <tr>
    ...
    <td>
      <ul style="margin: 0; padding-left: 10px;">
        ...
        <li>비정상 작동에 예민한 도메인이므로, 작업 트랜잭션의 원자성에 항상 주목하였습니다.</li>
        <li>금전 거래 관련 사안 및 카지노측 제공 API와의 연동 오류에 대한 부분에 특히 신중한 개발 및 검증 과정을 거쳤습니다.</li>
        <li>선착순 이벤트 개발건에서의 동시성 처리 이슈를 락과 트랜잭션 처리를 통해 해결하였습니다.</li>
      </ul>
    </td>
  </tr>
  <tr>
    ...
    <td >
      <ul style="margin: 0; padding-left: 10px;">
        ...
        <li>이벤트 조회 성능의 이슈가 있어 캐싱 처리를 하여 해결하여 평균적으로 2초 가량에서 300ms 가량으로 조회 성능이 향상되었습니다.</li>
        <li>클라이언트 및 타 업체에서 제공받은 인터페이스와의 연동 과정에서의 특정 모듈의 Failure시 원자성 유지 및 롤백 처리에 집중하였습니다.</li>
      </ul>
    </td>
  </tr>
</table>
